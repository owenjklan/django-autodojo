...
class AutoDojoViewGenerator:
    ...
    default_docstring = """
        Automatically generated endpoint for {http_verb} requests to update {model}
        objects.
    ...
    def patch_doc_string(self, view_func: Callable, docstring: str = None) -> Callable:
        used_docstring = ""
        if docstring is not None:
            used_docstring = docstring
        elif hasattr(self, "default_docstring"):
            if not isinstance(self.default_docstring, str):
                raise TypeError(
                    f"{self.__class__.__name__}.default_docstring must be a string"
                )
            used_docstring = self.default_docstring

        http_verb = SPECIAL_METHODS_TRANSLATION.get(
            self.http_method, self.http_method
        ).upper()

        view_func.__doc__ = used_docstring.format(
